...
export class LeaveTransferContract extends Contract {
    ...
    @Transaction()
    public async CreateLeave(ctx: Context, datas: string): Promise<void> {
        const hrmLeaveData = JSON.parse(datas);
        for (const data of hrmLeaveData) {
            await ctx.stub.putState(data.ID, Buffer.from(JSON.stringify(data)));
            console.info(`HRM leave Data ${data.ID} initialized`);
        }
    }

    // ReadAsset returns the asset stored in the world state with given id.
    @Transaction(false)
    public async ReadLeave(ctx: Context, id: string): Promise<string> {
    ...
    @Transaction()
    public async TransferLeave(ctx: Context, id: string, newOwner: string): Promise<void> {
    ...
    @Transaction(false)
    @Returns('string')
    public async GetAllLeave(ctx: Context): Promise<string> {
        const allResults = [];
